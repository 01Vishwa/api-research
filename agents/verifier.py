...
def _get_llm():
    ...
    return ChatOpenAI(
        ...
        temperature=0.0,              # Verifier must be deterministic
...
def fetch_evidence_node(state: VerifierState) -> VerifierState:
    ...
    if url:
        page_text = fetch_page_sync(url) or ""
    ...
    if not page_text or len(page_text) < 200:
        ...
        if results:
            ...
            for r in results:
                text = fetch_page_sync(r.get("url", "")) or ""
                if len(text) > 300:
                    page_text = text
    ...
    return {**state, "page_text": page_text}
...
Now extract the integration facts for {app_name} from the above documentation only."""
    ...
    try:
        ...
        return {**state, "re_derived": {}, "error": str(exc)}
...
def diff_compare_node(state: VerifierState) -> VerifierState:
    ...
    log = VerificationLog(
        ...
        verifier_notes=derived.get("source_quote", ""),
    )
